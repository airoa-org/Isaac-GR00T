...
class TorchProfilerCallback(TrainerCallback):
    def __init__(self,
        ...
        self._end_global_step = None

    def on_train_begin(self, args, state, control, **kwargs):
        os.makedirs(self.save_dir, exist_ok=True)
        self._enabled = True
        self._start_global_step = state.global_step + self.warmup_steps
        self._end_global_step = self._start_global_step + self.active_steps
        # W&B にメタを書いておくと後から比較が楽
        if wandb.run is not None:
            wandb.run.summary[f"profile_start_step_{self.schedule_label}"] = self._start_global_step
            wandb.run.summary[f"profile_active_steps_{self.schedule_label}"] = self.active_steps

    def on_step_begin(self, args, state, control, **kwargs):
        if not self._enabled:
            return
        step = state.global_step
        # アクティブ区間に入ったら Prof を起動
        if step == self._start_global_step:
            self._prof = profile(
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if not self._enabled or self._prof is None:
            return
        self._prof.step()
        step = state.global_step
        # アクティブ区間終了で閉じる
        if step >= self._end_global_step:
            try:
                self._prof.__exit__(None, None, None)
                # 主要カーネルのテーブルをログに出す（rank0のみを想定）
                table = self._prof.key_averages().table(
                    sort_by="self_cuda_time_total", row_limit=30
                )
                print("\n=== PROFILER TOP (self_cuda_time_total) ===\n", table)
                if wandb.run is not None:
                    wandb.log({f"profiler/top_{self.schedule_label}": wandb.Html(f"<pre>{table}</pre>")})
                # トレースを zip で固めて W&B artifact としてアップ（後からダウンロード/比較用）
                zbuf = io.BytesIO()
                ...
                zbuf.seek(0)
                art = wandb.Artifact(f"profiler_{self.schedule_label}", type="trace")
                with art.new_file("tb_logs.zip", mode="wb") as f:
                    f.write(zbuf.read())
                wandb.run.log_artifact(art)
            finally:
                self._prof = None
                self._enabled = False  # 一回だけ計測
...
def main(config: ArgsConfig):
    ...
    prof_cb = TorchProfilerCallback(
        warmup_steps=10,
        active_steps=40,
        save_dir="./tb_logs_bs_probe",
        schedule_label=f"bs{config.batch_size}"
    )

    # 2.3 run experiment
    experiment.trainer.add_callback(prof_cb)
    experiment.train()
