...
class ArgsConfig:
    ...
    pa_instruction: str = "open the door"
...
def sanity_check(ds, include_episodes: set[int], max_print: int = 10):
    ...
    assert ids == set(include_episodes), f"ミスマッチ: ds={sorted(ids)} vs req={sorted(include_episodes)}"
    assert len(ds.all_steps) == int(sum(ds.trajectory_lengths)), "all_steps と lengths の不整合"

    print(f"[OK] trajectories: {len(ids)} 本, 合計ステップ={len(ds)}")
    print("一部ID:", sorted(list(ids))[:max_print])

def probe_samples(ds, n=50):
    bad = []
    for i in np.random.default_rng(0).integers(low=0, high=len(ds), size=n):
        sample = ds[i]
        ep = int(sample["episode_index"])
        img = sample["eagle_content"]["image_inputs"][0]
        img.save(f"{i}.png")
        if ds.include_episodes is not None and ep not in ds.include_episodes:
            bad.append((i, ep))
    if bad:
        raise AssertionError(f"選外エピソードが混入: {bad[:5]} ... total={len(bad)}")
    print(f"[OK] random probe {n} samples all within include_episodes")
...
def main(config: ArgsConfig):
    ...
    import json
    include_eps = json.load(open("/home/group_25b505/group_6/workspace/user_00031_25b505/Isaac-GR00T/microwave-aist.json","rb"))[config.pa_instruction]
    ...
    #sanity_check(train_dataset, include_eps)
    #probe_samples(train_dataset, n=200)
