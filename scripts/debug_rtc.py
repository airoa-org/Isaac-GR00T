...
def build_rtc_weight_mask(H: int, d_steps: int, s_steps: int, lam: float,
    ...
    """
    # 例: 中間長 = H - d - s
    mid_len = max(0, H - d_steps - s_steps)
    w_time = np.zeros((H,), dtype=np.float32)
    if d_steps > 0:
        w_time[:d_steps] = 1.0
    if mid_len > 0:
        t = np.arange(mid_len, dtype=np.float32)
        w_time[d_steps:d_steps + mid_len] = np.exp(-lam * t)  # 単純な指数減衰
    # 末尾 s は 0.0 のまま

    W = torch.from_numpy(w_time).to(device=device, dtype=dtype)[None, :, None]  # [1,H,1]
    W = W.expand(B, H, D).contiguous()  # [B,H,D]
    return W
...
def main():
    ...
    ap.add_argument("--s-ratio", type=float, default=0.5)  # s_steps = int(H * s_ratio)
    ...
    def one_pass(use_rtc_flag: bool) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for step in range(steps):
            ...
            if not use_rtc_flag:
                ...
                s_steps = max(int(H * args.s_ratio), d_steps)
                W = build_rtc_weight_mask(H, d_steps, s_steps, lam=args.lam,
    ...
    # ベースライン（RTCなし）
    errs0, chunk_errs0 = one_pass(use_rtc_flag=False)
    ...
    # RTCあり
    errs1, chunk_errs1 = one_pass(use_rtc_flag=True)
